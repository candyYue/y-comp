...
export const Id2FieldText = (text, fieldsList) => {
  ...
  text = text.replace(customRegex, ($1) => {
    ...
    const className = (field && field.type === 1) ? 'api-fields-color' : 'fields-color'
    // return field ? `@${field.title} ` : ''
    return field ? `<span data-id="${field.id}" data-key="field" class="${className}">@${field.title}</span><span data-key="gap">&nbsp;</span>` : ''
  })
  return text
}
export const Id2PureText = (text, fieldsList, hasPre = true) => {
  const customRegex = /\${[1-9]\d*}/g
  text = text.replace(customRegex, ($1) => {
    const id = Number($1.slice(2, $1.length - 1))
    const field = fieldsList.find(d => d.id === id)
    return field ? `${hasPre ? '@' : ''}${field.title} ` : ''
  })
  return text
}
