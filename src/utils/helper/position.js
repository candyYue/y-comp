export function getCursorXY (input, caretPosition) {
  // const input = e.target
  const inputDimensions = input.getBoundingClientRect()
  const { left: inputX, top: inputY, width: inputWidth } = inputDimensions
  // const inputValue = input.value
  const inputValue = input.innerText
  const div = document.createElement('div')
  const span = document.createElement('span')
  div.classList.add('el-textarea__inner')
  div.classList.add('el-textarea__copy')
  div.style.width = inputWidth + 'px'
  div.innerText = inputValue.substr(0, caretPosition)
  span.innerText = '.'
